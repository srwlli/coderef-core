...
```
 ...
 │  → ValidationResult { ok, errors[], warnings[], report (12 fields) }│
...
2. **`ValidationReport` (the `.coderef/validation-report.json` file)** — the 12-field locked report from Phase 6 (`header_coverage_pct` added additively by WO-RAG-HEADER-COVERAGE-ENFORCE-AND-SURFACE-001). Field names are additive-only.
...
- `report` — the 12-field locked `ValidationReport` (R-PHASE-6-C; 12th field `header_coverage_pct` additive).
