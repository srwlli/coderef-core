...
`coderef-mcp-server` (MCP domain `coderef-core`, registered via `.mcp.json`) exposes the artifacts above as 33 tools — see the tool table in [docs/CLI.md § coderef-mcp-server](./CLI.md#coderef-mcp-server) for the authoritative list. Every tool requires `project_root` (the server is repo-agnostic; there is no default repo). Most are read-only (`what_calls`, `what_imports`, `impact_of`, `find_element`, `codebase_summary`, `validation_status`, `hotspots`, `cycles`, `what_exports`, `diff_impact`, `tests_for_change`, `ast_search`, `type_hierarchy`, `dependency_rules`, `docstrings`, `clones`, `rag_search`, `pack_context`, `rename_preview`, `map`, …); `reindex`, `rag_index`, and `api_diff` (snapshot mode) write, confined to `<project_root>/.coderef/`. If your runtime is MCP-capable, use these instead of reading `graph.json`/`index.json` directly — the server is typed against `ExportedGraph`, traverses only `resolved` edges, and returns ambiguity envelopes (≤5 candidates) rather than guessing. `validation_status` returns the 14-field report verbatim.
