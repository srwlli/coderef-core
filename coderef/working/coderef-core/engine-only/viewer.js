/* coderef map viewer engine for engine-only */
(function () {
  'use strict';
  ...
  var PALETTE = ['#bc8cff', '#58a6ff', '#3fb950', '#d29922', '#f0883e', '#ff7b72'];
  var layerColorMap = {
    'presentation': '#bc8cff',
    'service': '#58a6ff',
    'pipeline': '#3fb950',
  ...
  function init(data) {
    nodes = data.nodes.map(function (n) {
      return Object.assign({}, n, {
        ...
        vx: 0, vy: 0,
        r: 8 + Math.sqrt((n.elementCount || 0)) * 2,
        color: layerColorMap[n.layer] || '#58a6ff',
        fixed: false
      });
    });
    ...
    edges.forEach(function (e) {
      ...
      outAdj.get(e.source).push({ node: e.b, weight: e.weight || 1 });
      inAdj.get(e.target).push({ node: e.a, weight: e.weight || 1 });
    });

    document.getElementById('repo-name').textContent = data.meta.repoName || 'coderef-intel-engine';
    document.getElementById('stats').textContent =
      nodes.length + ' modules · ' + edges.length + ' edges · ' +
  ...
  function draw() {
    ...
    for (i = 0; i < edges.length; i++) {
      ...
      ctx.strokeStyle = e.kind === 'external' ? 'rgba(139,148,158,0.35)' : 'rgba(88,166,255,0.55)';
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(p.x, p.y);
      ctx.lineTo(q.x, q.y);
    ...
    for (i = 0; i < nodes.length; i++) {
      ...
      var r = n.r * view.k;
      ctx.beginPath();
      ctx.arc(p.x, p.y, Math.max(4, r), 0, Math.PI * 2);
      ...
      if (n === selected || n === hovered || view.k > 0.7) {
