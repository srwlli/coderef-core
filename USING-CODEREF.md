...
- **Docs are graph nodes at SECTION grain, and doc→code links are gated (WO-TREAT-MARKDOWN-FILES-LIKE-CODE-SECTION-LEVEL-AST-001).** Each ingested markdown doc keeps its whole-file `@Doc/<path>` node and additionally exposes `@Doc/<path>#<slug>` section nodes linked by `contains`. A section names code via a `references` edge ONLY when the symbol table backs the name — so **absence of a `references` edge is not "the docs don't mention it"**: a mention CodeRef cannot back is silently not a claim, and an ambiguous one is recorded `ambiguous` with candidates rather than guessed. Two deliberate quarantines: identifiers inside fenced ` ```ts `/` ```js ` examples never enter the symbol table (a doc example calling `run()` cannot mint a call edge, move resolution-rate, or reach `rename --apply`), and `references` is NOT walked as a dependency — `impact_of` answers are unchanged by doc content. Ask for doc impact explicitly via `CanonicalGraphQuery.docReferences(query)`. Section ids are stable under cosmetic heading edits but a genuine heading RENAME re-keys the section, the same trade-off `codeRefId` makes with line numbers.
