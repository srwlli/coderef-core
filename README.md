...
Real-world self-scan baseline (post WO-EDGE-RESOLUTION-IMPROVEMENT-PROGRAM-001 P1 test_dsl reclassify, 2026-08-01): `valid_edge_count=9927`, `resolved_of_resolvable=73.38%` (the honest resolver-quality read; raw all-calls `resolution_rate=22.42%` includes builtin/external by design), `unresolved_count=1871` (was 18,527 — 16,717 test-DSL calls now classify `builtin` with disclosed `test_dsl_count`), headers ~98% coverage, dependency cycles **0**.
