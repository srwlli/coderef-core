...
status: living
updated: 2026-07-31
documents: src/cli/coderef-mcp-server.ts (37-tool MCP registry) + docs/CLI.md (26 CLI entrypoints)
doc_type: guide
...
5. **EDIT** (structural changes) — RUN `ast_search` to enumerate every call-site shape; `rename_preview` before any rename; `type_hierarchy` before touching an interface (`item_format:"lsp"` / CLI `--lsp` additively emits LSP 3.17 `TypeHierarchyItem`s for editor-shaped consumers). **Write-scope rule:** graph tools never write source — `rename --apply` is CLI-only by design; MCP exposes `rename_preview` only. The `/coderef-rename` skill is the EDIT-leg front-door for the apply gate (preview → review → `--apply`, then re-populate because the graph is now stale). Your other edits go through your editor tools; the graph tells you where.
