/* coderef viewer engine for parsed-surfaces */
(function () {
  'use strict';
  function boot() {
  ...
  var layerColorMap = { 'map-viewer': '#58a6ff', 'doc-gen': '#bc8cff', 'context-pack': '#3fb950', 'exporter': '#d29922' };
  ...
  function init(data) {
    ...
    document.getElementById('repo-name').textContent = data.meta.repoName || 'parsed-surfaces';
    document.getElementById('stats').textContent = nodes.length + ' systems · ' + edges.length + ' edges';
  ...
  function simulate() {
    ...
    for (i = 0; i < nodes.length; i++) {
      ...
      for (j = i + 1; j < nodes.length; j++) {
        ...
        if (d2 > 300000) continue; f = 3000 / d2;
    ...
    for (i = 0; i < edges.length; i++) {
      ...
      f = 0.02 * (d - 120); dx = dx / d * f; dy = dy / d * f;
  ...
  function renderDetail() {
    ...
    addRow(meta, 'System Layer', selected.layer); addRow(meta, 'Description', selected.sub || 'Downstream consumer');
  }
