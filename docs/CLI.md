...
| [`coderef-mcp-server`](#coderef-mcp-server) | Repo-agnostic MCP stdio server exposing `.coderef` intelligence as 32 tools (read + `.coderef`-write); `project_root` required per call | `--project-dir` (anchor) |
...
MCP (Model Context Protocol) stdio server that exposes `.coderef/` intelligence artifacts as 32 tools. Lets MCP clients (Claude Code, Claude Desktop, any MCP-compatible agent) query call graphs, impact analysis, and element lookups directly instead of parsing `graph.json` by hand.
...
| `docstrings` | Per-element docstring surface: the leading `/** */` JSDoc for JS/TS/JSX or the first string-literal statement for Python, attributed to a `codeRefId`, plus a coverage roll-up `{total, documented, undocumented, coverageRatio}`. Filter by name substring (`element`) and/or `documented` true\|false. Surfaces, NOT verdicts: `coverageRatio` is provenance, not a quality grade. Absence = no-data (`hasDocstring:false` with text omitted; empty index → `no_data:true`). Complements — does not replace — the file-grain JSDoc coverage in the docs analyzer |
...
| `docstrings` | Per-element docstring surface: the leading `/** */` JSDoc for JS/TS/JSX or the first string-literal statement for Python, attributed to a `codeRefId`, plus a coverage roll-up `{total, documented, undocumented, coverageRatio}`. Surfaces, NOT verdicts: `coverageRatio` is provenance, not a quality grade. Absence = no-data (`hasDocstring:false`; empty index → `no_data:true`). Complements the file-grain JSDoc coverage in `--type=docs`. | opt: `--element` (name filter), `--documented`/`--undocumented`, `--limit`, `--offset` |
