...
**Markdown is scanned at section grain** (WO-TREAT-MARKDOWN-FILES-LIKE-CODE-SECTION-LEVEL-AST-001). `collectDocFacts` (Phase 5's docs lane, see `src/pipeline/doc-ingest.ts`) parses each ingested doc's heading hierarchy into `@Doc/<path>#<slug>` **section nodes**, additive children of the retained whole-file `@Doc/<path>` node and joined to it by a resolved `contains` edge carrying `{depth, order}` — enough to rebuild a document's outline from edges alone. Headings inside fenced code blocks and inside frontmatter are not headings; duplicate slugs disambiguate `-2`/`-3` in document order.

Sections then name code through **one membership gate**: an identifier-shaped backtick span (prose lane) or an identifier lexed from a fenced `ts`/`js` block (code-block lane) mints a `references` edge **only when the symbol table backs the name**. The two miss modes are asymmetric on purpose — a name with no match mints nothing (backticks wrap far more paths and flags than symbols, and recording every non-claim would flood the graph), while a name matching several elements mints an `ambiguous` edge carrying all candidates, because there the claim is real and only the target is uncertain. `evidence.origin` keeps the two lanes distinguishable: a stale sentence and a stale example are different findings.

**Three** quarantines keep this additive, and it takes all three. Fenced-snippet identifiers **never enter the global symbol table** — they are gate candidates and nothing more, so a doc example calling `run()` cannot mint a call edge into real code, move the resolution-rate denominator, or come within reach of `rename --apply`. `references` is **excluded from the dependency-traversal sets** on both adjacency indexes, since it targets ordinary code elements and walking it would change what `impact_of` answers for every symbol a doc mentions; `contains` **is** in those sets, because it can only ever land on doc nodes. And doc section nodes are **excluded from name-keyed resolution** in both resolvers (`CanonicalGraphQuery.resolve` and the MCP `resolveNodes` twin), in the exact-name branch and the substring fallback alike.

That third one was learned the hard way. The first two reason about which EDGES get walked, and both are sound — but a section's `name` is its heading text, and a resource sheet routinely heads a section with the exact name of the symbol it documents. Name lookup therefore matched the prose section alongside the real element and perturbed the SEED SET, before any traversal began, silently changing `what_calls` / `impact_of` / `find_element` / `symbol_context` for real code. It shipped and was fixed in `daf4899`. **A new node kind has two blast surfaces — which edges are walked, and what name lookup returns** — and edge-level reasoning is blind to the second. Sections stay addressable by exact id and reachable via `contains` / `docReferences()`.

Doc facts stay out of the incremental per-file fact store (the surface is tiny and repo-global, so both pipeline paths re-collect it fresh), which is what makes full-vs-incremental parity hold by construction for sections and references alike.
