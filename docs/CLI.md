...
| [`coderef-mcp-server`](#coderef-mcp-server) | Repo-agnostic MCP stdio server exposing `.coderef` intelligence as 33 tools (read + `.coderef`-write); `project_root` required per call | `--project-dir` (anchor) |
...
MCP (Model Context Protocol) stdio server that exposes `.coderef/` intelligence artifacts as 33 tools. Lets MCP clients (Claude Code, Claude Desktop, any MCP-compatible agent) query call graphs, impact analysis, and element lookups directly instead of parsing `graph.json` by hand.
