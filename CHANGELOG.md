...
- **Query surface**: `CanonicalGraphQuery.docReferences(query)` answers "which prose names this symbol". Keeping doc content out of existing answers took TWO exclusions, not one: `references` is deliberately not in either adjacency index's dependency set (`contains` is — it can only land on doc nodes), AND doc section nodes are excluded from name-keyed resolution in both resolvers. The second was missed at first and shipped as a defect: resource sheets head sections with the exact name of the symbol they document, so `resolve('normalizeSlashes')` returned the prose section alongside the function and silently changed `what_calls` / `impact_of` / `find_element` / `symbol_context` for real code. Fixed in `daf4899`; sections remain addressable by exact id. Verified differentially against a reconstructed pre-WO graph — 0 differing answers across 1209 query comparisons, including all 3 names that collide with a section heading.
- **Instrument note**: the frozen-tree A/B below compares GRAPH CONTENT and is structurally blind to query semantics. It passed byte-identical at all three phases while the resolution defect above was live. A new node KIND has two blast surfaces — which edges are walked, and what name lookup returns — and only the first was gated.
