...
- **`coderef-mcp-server`** — an MCP stdio server (registered as the `coderef-core` domain in `.mcp.json`) exposing 23 intelligence tools over `.coderef/` artifacts. Most are read-only (`what_calls`, `what_imports`, `impact_of`, `find_element`, `codebase_summary`, `validation_status`, `what_exports`, `hotspots`, `cycles`, `diff_impact`, `rag_search`, `pack_context`, `rename_preview`, `rag_status`, …); two (`reindex`, `rag_index`) WRITE, but every write is confined to `<projectDir>/.coderef/` (they delegate to the populate / rag-index pipelines — never source mutation). Source-mutating rename stays CLI-only; MCP exposes it as the dry-run `rename_preview`. Typed against `ExportedGraph`, so graph-schema drift is a compile error. See [docs/CLI.md](docs/CLI.md#coderef-mcp-server).
