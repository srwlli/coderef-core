...
- **Edges carry confidence tiers.** The honest resolver-quality read is `resolved_of_resolvable` (~74.8% post test_dsl reclassify + external-receiver dispositions + heritage-chain lookup, 2026-08-01; incremental `populate --changed-files` output is now byte-identical to a full rebuild — parity proven, edge-resolution P4): test-framework DSL calls (vitest/jest ambient globals + expect matcher chains) classify `builtin` with `test_dsl_*` reasons, disclosed via `test_dsl_count` — they were never resolvable and no longer pollute the denominator. Inherited and `super.` method calls now resolve EXACT through the declared extends/implements chain (`heritage_method_lookup`). The raw `resolution_rate` (~22%) keeps its all-emitted-calls denominator (includes builtin/external) by design. `scip_resolution_delta` shows what a compiler-grade index resolves that CodeRef didn't (read-only surface). The LIVE wire now exists (STUB-BQQJSY, opt-in): `populate-coderef --scip <path-to-.scip>` runs a post-resolution overlay that flips co-located unresolved/ambiguous **call** edges to `resolved` with SCIP provenance (`evidence.kind:'scip'`, confidence tier **heuristic** — SCIP resolved the *symbol*, not a proven intra-project `targetId`). No-regress by construction: already-resolved edges are never touched, no edges are invented, and no `.scip` = zero change. Generate the index locally with `npx @sourcegraph/scip-typescript index` (no cloud). Cross-repo resolution is still out of scope (STUB-6PGFZ3).
- **Cross-repo scope is OPT-IN and query-time (WO-CROSS-REPO-WORKSPACE-LINKAGE-001, closes STUB-6PGFZ3).** Zero cross-repo edges are ever PERSISTED. With a repo-local `.coderef/workspace.json` (`{version, packages:{"<pkg>":"<sibling-root>"}}`), workspace-mapped bare imports stay `external` but carry `workspacePackage`/`workspaceRoot` on their evidence, and `impact_of workspace:true` / `coderef-query --workspace` STITCH sibling graphs at query time (per-sibling outbound/inbound links). Trust rules for the stitch: it is package-grain (files+packages, not symbols); an absent sibling graph is a DISCLOSED skip, never "no dependents"; each sibling block carries its own `graph_generated_at` — sibling staleness is the sibling's, refresh it there. No registry = the old single-repo behavior, byte-identical.

- **Docs are graph nodes at SECTION grain, and doc→code links are gated (WO-TREAT-MARKDOWN-FILES-LIKE-CODE-SECTION-LEVEL-AST-001).** Each ingested markdown doc keeps its whole-file `@Doc/<path>` node and additionally exposes `@Doc/<path>#<slug>` section nodes linked by `contains`. A section names code via a `references` edge ONLY when the symbol table backs the name — so **absence of a `references` edge is not "the docs don't mention it"**: a mention CodeRef cannot back is silently not a claim, and an ambiguous one is recorded `ambiguous` with candidates rather than guessed. Three deliberate quarantines keep doc content out of answers about code: identifiers inside fenced ` ```ts `/` ```js ` examples never enter the symbol table (a doc example calling `run()` cannot mint a call edge, move resolution-rate, or reach `rename --apply`); `references` is not walked as a dependency; and **doc section nodes are excluded from name-keyed resolution** — a section's name is its heading, and sheets routinely head a section with the documented symbol's exact name, so without this a name lookup returns the prose alongside the function (that shipped as a defect and was fixed in `daf4899`). Sections are addressable by exact id, never by name. Ask for doc impact explicitly via `CanonicalGraphQuery.docReferences(query)`. Section ids are stable under cosmetic heading edits but a genuine heading RENAME re-keys the section, the same trade-off `codeRefId` makes with line numbers.
