...
Two quarantines keep this additive. Fenced-snippet identifiers **never enter the global symbol table** — they are gate candidates and nothing more, so a doc example calling `run()` cannot mint a call edge into real code, move the resolution-rate denominator, or come within reach of `rename --apply`. And `references` is deliberately **excluded from the dependency-traversal sets** on both adjacency indexes, since it targets ordinary code elements and walking it would change what `impact_of` answers for every symbol a doc mentions; doc impact is asked for explicitly via `CanonicalGraphQuery.docReferences()`. `contains` **is** in those sets — it can only ever land on doc nodes, so it cannot perturb an element-to-element answer.
