...
**SCIP-overlay edges are `resolved` with a REAL, node-set-verified `targetId` (STUB-BQQJSY, opt-in `--scip`).** The opt-in SCIP resolution overlay (`populate-coderef --scip <path>`) flips co-located unresolved/ambiguous **call** edges to `resolutionStatus:'resolved'` using a compiler-grade SCIP index — but ONLY when the reference's SCIP symbol can be followed to its definition occurrence in the same index AND that definition site lands on exactly one graph node. That node's real `id` is stamped as the edge's `targetId`, so GI-3 (resolved ⇒ targetId defined) and GI-2 (targetId ∈ node id set) hold by construction and the flipped edge is genuinely traversable by graph consumers (`what_calls`, `impact_of`, `tests_for_change`). The edge carries `evidence.kind:'scip'`, `evidence.scipSymbol`, `evidence.confidence:'provisional'`, `reason:'scip_resolved'`; its confidence tier is `heuristic` (resolved + provisional), never `exact` — the definition-site → node join is positional (line-grain), not an identity proof. A reference whose symbol has no unique node mapping flips **nothing** (the edge keeps its honest unresolved/ambiguous status; the miss is counted as `no_target_mapping`). Already-resolved edges are never touched and no edges are invented — a `--scip` run is a strict superset of a plain run's resolved set. Without `--scip`, none of this appears (byte-identical graph).
...
`coderef-mcp-server` (MCP domain `coderef-core`, registered via `.mcp.json`) exposes the artifacts above as 36 tools — see the tool table in [docs/CLI.md § coderef-mcp-server](./CLI.md#coderef-mcp-server) for the authoritative list. Every tool requires `project_root` (the server is repo-agnostic; there is no default repo). Most are read-only (`what_calls`, `what_imports`, `impact_of`, `find_element`, `codebase_summary`, `validation_status`, `hotspots`, `cycles`, `what_exports`, `diff_impact`, `tests_for_change`, `change_dossier`, `ast_search`, `type_hierarchy`, `dependency_rules`, `docstrings`, `clones`, `scip_resolution_delta`, `rag_search`, `pack_context`, `rename_preview`, `map`, `orient`, …); `reindex`, `rag_index`, and `api_diff` (snapshot mode) write, confined to `<project_root>/.coderef/`. If your runtime is MCP-capable, use these instead of reading `graph.json`/`index.json` directly — the server is typed against `ExportedGraph`, traverses only `resolved` edges, and returns ambiguity envelopes (≤5 candidates) rather than guessing. `validation_status` returns the 14-field report verbatim.

**The server states this contract itself.** Since WO-CODE-INTELLIGENCE-LEVERAGE-WIRING-PROGRAM-001 P1, the `initialize` handshake carries an `instructions` string (SDK `ServerOptions.instructions`) with the six load-bearing rules: `project_root` required per call; populate-first (`reindex` or `populate-coderef` before anything else on an unindexed/stale repo — the error envelope's missing-`.coderef` hint means you skipped this); orient with `map format:"skeleton"` + `codebase_summary` before grepping; prefer graph tools over grep for structure questions; surfaces-not-verdicts / absence-is-no-data; and the **write-scope rule** below. MCP clients receive it automatically; off-MCP consumers get the identical rules from this document.

**Write scope (explicit, by design — not an omission).** No MCP tool writes source files. The only source-writing operation in the toolchain is `coderef-rename --apply`, which is **CLI-only**: MCP deliberately exposes `rename_preview` (the read-only site list with confidence tiers) and stops there, so an agent driving the server can never mutate source through it. Index writes (`reindex`, `rag_index`, `map`, `api_diff` snapshot mode) are confined to `<project_root>/.coderef/`. If a workflow needs the rename applied, that is an operator/CLI action: `coderef-rename <old> <new> --apply` after reviewing the preview.

**`.coderef/rules.json` is the ONE committed file under `.coderef/`.** Everything else in that directory is generated output (gitignored via a contents-glob + negation); `rules.json` is declared architecture — the forbid layer-pair constraints the `dependency_rules` tool / `coderef-analyze --type=dependency-rules --gate` check against the observed `@layer` edge matrix. This repo dogfoods its own gate in CI (`npm run arch:gate` after a populate step). Do not regenerate or overwrite it from scan output — it is source-of-truth, edited by humans/agents like any other config.

**Self-scan scope is DECLARED — absence of a noise file is out-of-scope-by-design, not deletion.** This repo's own `.coderefignore` excises non-product noise from its self-scan universe (demo/example fixtures, `scripts/doc-gen/` tooling, dead-letter research scripts, the map-viewer bundle, the orphaned root `utils/fs.*` twin pair — WO-DECOMPOSE-CODEREF-MCP-SERVER-MONOLITH-001 P5). Those files remain on disk and in git; they are simply not in `index.json`/`graph.json`/the map universe, so do not read their absence as "deleted" or as no-data-by-accident. Pattern authoring carries a load-bearing trap: `shouldIgnorePath` matches each walked entry's bare basename at ANY depth, so a bare directory name (`utils/`) would drop `src/utils/` too — only path-qualified patterns (interior slash) or root-unique-verified filenames are allowed, guarded by `__tests__/pipeline/coderefignore-dogfood.test.ts`. The read-layer demo filter (`hotspots` `src_only`) is unchanged and still protects scans of OTHER repos.

**`type_hierarchy` speaks LSP on request.** `item_format:"lsp"` (MCP) / `--lsp` (`coderef-analyze --type=type-hierarchy`) additively attaches an LSP 3.17 `TypeHierarchyItem` projection — the seed element as the `prepareTypeHierarchy` item plus `typeHierarchy/supertypes` / `typeHierarchy/subtypes`-shaped arrays (`name`, numeric `SymbolKind`, `file://` `uri`, 0-based whole-line `range`/`selectionRange`; coderef provenance rides the spec's opaque `data` slot). `range` spans the element via the index's `endLine` substrate; older indexes degrade to a DISCLOSED single-line range (`range_source:"line_only"` + `degraded_single_line`), and unresolved heritage endpoints (no file, so no spec-valid `uri`) are excluded and counted in `excluded_unresolved` — missing data, never "no such supertype". The default envelope is byte-identical when the option is absent.
...
**Zero test in-edges ≠ untested — check `testLinkage.subprocess`.** CLI-heavy repos exercise dist bins through child_process spawns, which the static graph cannot see. The map's `metrics.testLinkage` carries an ADDITIVE `subprocess` block (default-on at map generation, the one impure test-file read at the generateMap edge — git-history pattern): spawn/exec string literals in test files referencing `dist/src/**` bins, mapped back to their src files (`linked`: src file → `{testFileCount, refCount, testFiles[]}`). The `summary` keys are untouched — `srcWithoutTestEdgeCount` still counts direct resolved graph edges only, so `map_metrics_delta` semantics are unchanged. Read `subprocess.linked` before treating a zero-edge file as uncovered. Absence of the block = no data (no test contents scanned), never zero.
