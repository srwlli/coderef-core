...
| Field | Contents |
|-------|----------|
| `testLinkage` | `summary` `{testFileCount, srcFileCount, srcWithTestEdgeCount, srcWithoutTestEdgeCount}`; `testFiles[]` (sorted, classified by the same heuristic graph-analytics uses — single source); `inboundFromTests` (src file → `{testFileCount, weight}`, key-sorted; absence = observed zero); `zeroTestInEdge[]` (sorted src files with no inbound test edge, capped, default 200); `subprocess` (ADDITIVE disclosure, present when test-file contents were scanned: `summary` `{testFilesScanned, testFilesWithSpawnRefs, linkedSrcFileCount, spawnRefCount}` + `linked` src file → `{testFileCount, refCount, testFiles[]}` — spawn/exec literals in test files referencing `dist/src/**` bins mapped back to src. The summary keys above stay UNTOUCHED (direct resolved graph edges only); block absence = no-data, never zero). |
...
**Scan scope (`.coderefignore`).** The file walk honors `DEFAULT_PIPELINE_IGNORE_PATTERNS` (node_modules, dist, `.coderef/`, caches, minified bundles) plus an optional project-root `.coderefignore` — gitignore-style: one pattern per line, lines starting with `#` are comments (inline *trailing* comments are NOT stripped, so keep rationale on its own line). **Pattern-authoring trap (load-bearing):** `shouldIgnorePath` matches each walked entry's *bare basename* as well as its root-relative path, so a bare directory name (`utils/`) excludes every directory with that basename at ANY depth — including `src/utils/`. Author path-qualified patterns (interior slash, e.g. `scripts/doc-gen/`) or filenames verified unique to the repo root. Bare names are appropriate only for build-artifact dirs you want dropped everywhere (`dist/`, `coverage/`). coderef-core dogfoods this: its own `.coderefignore` excises non-product noise (demo/example fixtures, doc-gen tooling, dead-letter scripts, the root `utils/fs.*` twin pair) from its self-scan universe, guarded by `__tests__/pipeline/coderefignore-dogfood.test.ts` (WO-DECOMPOSE-CODEREF-MCP-SERVER-MONOLITH-001 P5).
...
| `api_surface` | The project's HTTP endpoint inventory as a GRAPH surface. Every detected endpoint (canonical identity `@Endpoint/<path>#<METHOD>` with parameter NAMES erased per the OpenAPI 3.1 path-identity rule, so a client's `${id}` interpolation and a server's `<int:user_id>` are ONE endpoint), the files that SERVE it, the files that CALL it over HTTP, plus `network_edges` — the file-to-file hops that cross the process boundary, deliberately kept OUT of the module-dependency edge set. `unmatched_calls` lists every client call that did NOT bind WITH the reason: `endpoint_not_in_project` (404-shaped), `endpoint_method_not_served` (405-shaped), `client_path_origin_unresolved` (an interpolated base URL whose origin is unknowable), `absolute_url_external_origin` (a different authority per RFC 3986 §3.2). Surfaces, NOT verdicts: `orphaned` means NO RESOLVED CALLER WAS FOUND IN THIS REPO — for a public API or a client in another repo that is the expected state, never a dead-code verdict. Absence = no-data (`routes.json` not produced → `no_data:true`), never a false "0 endpoints". Known bound: the frontend-call detector gates on browser-reachable extensions, so server-to-server callers are invisible to it |
...
| `tests_for_change` | Diff-to-test-selection in one call: map a git diff (default working tree vs HEAD) to changed elements, then return the TEST-FILE elements that reach them through resolved call/import edges, ranked by directness (depth 1 = direct). Also emits **`run_command`** — a ready-to-run line (e.g. `npx vitest run <ranked files>`) when a runner is detectable from package.json (`scripts.test` first, then declared runner deps); no detectable runner → `run_command:null` + `run_command_no_data`, never a guessed command. Absence is no-data, not "untested" |
...
| `type_hierarchy` | Class/interface supertypes + subtypes over the `extends`/`implements` heritage edges the pipeline populates. `direction:"up"` = ancestors (what the element extends/implements), `"down"` = descendants, `"both"` (default) = each; every hit carries depth (1 = direct) + heritage kind, attributed to a `codeRefId`. `item_format:"lsp"` additively attaches an LSP 3.17 `TypeHierarchyItem` projection (seed = prepare item; numeric SymbolKind, `file://` uri, 0-based whole-line range/selectionRange from the index's `endLine` substrate, degrading to a disclosed single-line range on older indexes; unresolved endpoints excluded + counted). Absence is no-data (no recorded heritage edge), never "flat hierarchy"; an unresolved external supertype is returned with `resolved:false` |
...
| `dependency_rules` | Dependency-rules gate: check DECLARED architecture constraints (optional `.coderef/rules.json` — `forbid`/`allow` layer-pairs `{from,to}`) against the OBSERVED declared-layer edges projected from `graph.json` (`@layer` headers). Per rule: `satisfied` \| `violated` \| `not_applicable`, with the offending edges named. Surfaces, NOT verdicts: no composite architecture-health score. Absence = no-data (no `rules.json` → `no_data:true`), never a false "all rules pass". Read-only report; the CI exit-code gate lives on the `coderef-analyze --type=dependency-rules --gate` CLI (MCP only reports). DOGFOOD: coderef-core ships its own committed `.coderef/rules.json` (7 forbid rules) and runs the gate in CI (`npm run arch:gate`) |
| `change_dossier` | The pre-commit pre-flight in ONE call: for a git diff (default working tree vs HEAD), compose the four verify surfaces — `diff_impact` (blast radius + top affected files), `tests_for_change` (ranked selection + `run_command`), `api_diff` in delta mode (exported-API change vector vs the baseline snapshot), and `dependency_rules` (declared-constraint mismatches) — into one condensed envelope. Surfaces, NOT verdicts: names WHERE to look before committing, never a merge decision (no composite risk score). Absence = no-data: a leg that could not run is named in `no_data` with a warning; a leg reporting its own `no_data:true` ran but had nothing to compare. Read-only (`api_diff` composed in delta mode only — never snapshots) |
...
| `clones` | Clone surface with THREE passes (`pass`, default `structural`). **structural**: groups elements sharing an identical signature `(kind, name, arity, sorted param-name shingle, sorted import-source set)` — renamed copies, boilerplate handlers, parallel helpers; no body data needed. **lexical**: groups elements whose persisted `normalizedBodyHash` (comment-stripped, whitespace-collapsed BODY hashed at extract time — name/signature excluded, so renamed copies match) is IDENTICAL — byte-level copy-paste and same-body-different-name clones, returned as `lexical_groups [{bodyHash, normalizedLength, members, size}]`. **near_miss** (opt-in, `similarity_threshold` default 0.9): pairs elements whose persisted `astFingerprint` vectors (named-AST-node-type counts, function-like kinds only) meet the threshold under normalized-L1 similarity, returned as `near_miss_pairs [{a, b, similarity}]`; candidates bucket by (kind, extension family) and identical-hash pairs are excluded (lexical territory). `min_body_length` excludes tiny bodies from the body passes (excluded count disclosed). Surfaces, NOT verdicts: a group/pair is CO-LOCATION, NOT a defect; `similarity` is measured provenance, not a grade. Absence = no-data: empty index — or a body pass over an index with NO persisted substrate (populated pre-substrate / regex-fallback scanner) — → `no_data:true`, with `elements_without_body_data` disclosing the gap (repopulate to refresh). `signature_basis` + `elements_without_signature` disclose the structural basis. Filter by name substring (`filter`) and/or `min_group_size` |
| `scip_resolution_delta` | What a user-provided SCIP index (from an external `scip-*` indexer) resolves that CodeRef's own tree-sitter heuristic did NOT. CodeRef resolves ~73% of resolvable calls (post test_dsl reclassify; raw all-calls rate ~22%); SCIP is compiler-grade. Returns delta rows `{codeRefId, scipSymbol, file, line, coderefStatus, scipStatus:'resolved', provenance:'scip'}` for each SCIP-resolved reference whose co-located CodeRef edge is unresolved/ambiguous/absent, plus a roll-up `{scip_documents, scip_occurrences, delta_resolved_by_scip, delta_ratio}`. OPT-IN via `scip_path`; absence = no-data (`no_data:true`, never a false "0 delta"). Surfaces, NOT verdicts: a delta is a resolution-provenance gain, not a grade. READ-ONLY: does NOT feed the resolver (the live SCIP-into-resolver wiring is a deferred deep integration needing a real `scip-typescript` index) |
...
| `what_this_calls` | Outbound twin of `what_calls`: which resolved call sites does this element invoke? (`file:line` locations + a `confidence` tier per callee) |
| `what_this_imports` | Outbound twin of `what_imports`: what does this element/module import? (resolved outbound import edges) |
| `what_this_depends_on` | What does this element depend on, transitively? Outbound call+import BFS — the forward mirror of `impact_of` |
| `find_all_references` | Every resolved mention of the element across edge kinds (calls, imports, exports, heritage) — broader than `what_calls`; the go-to before asserting "nothing references this" |
| `path_between` | How does A reach B? Directed path over resolved edges between two elements; fragment matches get an ambiguity envelope instead of a guess |
| `source_of` | The exact implementation: file, line range, and source snippet for an element — pull the code without opening the file |
| `unresolved_edges` | Where is the graph blind? The unresolved/ambiguous edge surface — the coverage companion to check before trusting any load-bearing negative |
| `pack_context` | Token-budgeted context assembly: ranked, deduplicated element/file excerpts filling a `token_budget` instead of hand-picking files; `include_callers` attaches ego-graph neighbors — see **Ego-graph expansion** below |
...
| `--lsp` | Additively attach the LSP 3.17 `TypeHierarchyItem` projection (used by: `type-hierarchy`) | off |
...
| `--ref=<ref>` | Git ref to diff against (used by: `tests-for-change`, `change-dossier`) | `HEAD` |
...
| `tests-for-change` | Diff-to-test-selection: which test-file elements reach the diff through resolved call/import edges, ranked by directness (depth 1 = direct). Emits `run_command` (ready-to-run line) when a runner is detectable from package.json; otherwise `run_command:null` + `run_command_no_data`. Absence is no-data, not "untested". | `--ref` (default `HEAD`) |
| `change-dossier` | Pre-flight envelope for a proposed change: blast radius (impact BFS) + ranked test selection (+ `run_command`) + exported-API delta vs the `baseline` snapshot + dependency-rule mismatches, composed for one git diff. Surfaces, not verdicts; absent legs named in `no_data`; read-only (never snapshots). CLI mirror of the MCP `change_dossier` tool. | `--ref` (default `HEAD`), `--depth` |
...
| `clones` | Clone surface with three passes (`--pass`, default `structural`): structural groups on `(kind, name, arity, param-name shingle, import-source set)`; lexical groups IDENTICAL persisted normalized-body hashes (byte-level copy-paste, same-body-different-name); near_miss pairs similar persisted AST fingerprints under `--similarity-threshold` (default 0.9; opt-in, Deckard-style; identical-hash pairs excluded). `--min-body-length` excludes tiny bodies from the body passes (disclosed, never silent). Surfaces, NOT verdicts: co-location, NOT a defect; near-miss `similarity` is provenance, no duplication score. Absence = no-data: a body pass over an index without the persisted substrate → `no_data:true` + `elements_without_body_data` (repopulate to refresh). Requires `--output=json` for parseable output. | opt: `--pass`, `--element` (name filter), `--min-group-size`, `--similarity-threshold`, `--min-body-length`, `--limit`, `--offset` |
| `scip-resolution-delta` | What a user-provided SCIP index resolves that CodeRef did NOT (CodeRef resolves ~73% of resolvable calls (post test_dsl reclassify; raw all-calls rate ~22%); SCIP is compiler-grade.. Delta rows for each SCIP-resolved reference whose co-located CodeRef edge is unresolved/ambiguous/absent + a `{delta_resolved_by_scip, delta_ratio}` roll-up. OPT-IN via `--scip=<.scip path>`; absence = no-data (`no_data:true`, never a false "0 delta"). Surfaces, NOT verdicts (resolution-provenance gain, no score). READ-ONLY — does NOT feed the resolver (live SCIP-into-resolver wiring is a deferred deep integration). Requires `--output=json`. | opt: `--scip=<path>`, `--limit`, `--offset` |
